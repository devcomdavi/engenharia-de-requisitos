...
## 3. Pré-condições
- O usuário deve estar autenticado no sistema (exceto para usuários públicos, onde não possuem acesso à Dashboard).
- O usuário deve possuir as permissões adequadas de acordo com seu cargo (Administrador, Moderador ou Capitão).

## 4. Fluxo Principal
1. O sistema exibe a interface correspondente para cargo logado: Administrador, Moderador ou capitão.
2. O ator acessa o menu principal e seleciona a funcionalidade desejada.
4. O ator aciona o botão de confirmação.
5. O sistema valida as regras de negócio e os dados informados.
6. O sistema processa a operação e atualiza a tela exibindo os resumos estatísticos.
...
- **[FA02] Permissão Negada**:
  - Caso o ator tente visualizar registros aos quais não possui escopo (ex: Capitão tentando vizualizar atleta de outra atlética), o sistema bloqueia a ação, retorna um erro de acesso negado e registra a tentativa em log.
